...
const Blog = () => {
  ...
  const filteredPosts = useMemo(() => {
    ...
    if (!searchTerm && selectedTag === "All") {
      ...
      searchTerm &&
      featuredPost &&
      posts.some((p) => p.id === featuredPost.id)
    ) {
      ...
      featuredPost &&
      posts.some((p) => p.id === featuredPost.id) &&
      !featuredPost.tags.includes(selectedTag)
    ) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
        ...
        <div className="container px-4 mx-auto">
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-lg text-muted-foreground max-w-2xl mb-8">
              Expert analyses, guides, and insights on blockchain security from
              the Arjuna audit team.
            </p>
      ...
      {(isFiltering || (!isFiltering && hasPostsToDisplay)) && (
        <section
          className={`py-12 ${!shouldShowFeaturedPost ? "pt-12" : "bg-black/30"}`}
        >
          {" "}
          {/* Adjust padding/bg if featured is hidden */}
          <div className="container px-4 mx-auto">
                  ...
                  {allTags.map((tag) => {
                    ...
                    if (isFilteringActive) {
                      if (isActive) {
                        ...
                        buttonClasses +=
                          " hover:bg-background hover:text-accent-foreground"; // Keep text color consistent with default outline hover text
                      }
                    }
              ...
              <div className="mt-12 flex justify-center">
                <nav className="flex items-center gap-1">
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    disabled={postsToDisplay.length < 10}
                  >
                    Next
                  </Button>
                </nav>
